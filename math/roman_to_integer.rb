...
# Therefore, the simplest algorithm is to use a pointer to scan through the string, at each step deciding whether to add the current symbol and go forward 1 place, or add the difference of the next 2 symbols and go forward 2 places.
...
def roman_to_int(s)
  ...
  s.chars.each_with_index do |el, i|
    ...
    if ROM_NUMS[s[i + 1]] && ROM_NUMS[el] < ROM_NUMS[s[i+1]]
      temp = ROM_NUMS[el]
    else # Else this is NOT the subtractive case.
