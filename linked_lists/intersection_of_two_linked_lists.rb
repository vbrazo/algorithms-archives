...
# at a time. When pApA reaches the end of a list, then redirect
# it to the head of B (yes, B, that's right.); similarly when pBpB
...
# Since B.length (=4) < A.length (=6), pBpB would reach the end of the
# merged list first, because pBpB traverses exactly 2 nodes less than
# pApA does. By redirecting pBpB to head A, and pApA to head B,
...
# So when pApA/pBpB reaches the end of a list, record the last element
